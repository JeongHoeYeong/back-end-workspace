...
public class Hello { // 클래스 명은 클래스 파일명과 동일해야 한다.
	...
	// 실행 메소드 (main 메소드) : 자바 프로그램 실행 시 가장 먼저 호출
	// 모든 클래스가 main 메소드를 가지고 있어야 하는 건 X
...
/*
 * * 네이밍 규칙
 * - 패키지명 : 소문자로 작성, 회사들 간에 패키지가 서로 중복되지 않도록
 *                        회사의 도메인 이름으로 패키지를 만든다.
 *                         
 * - 클래스명 : 대문자로 시작, 여러 개의 단어가 있을 때는
 *                        단어 앞자리마다 대문자로 표기
 * - 메소드명 : 소문자로 시작/ 여러 개의 단어가 있을 때는 안더 앞자리마다 대문자로 표기
 * 
 * 
 * * 자바의 특징
 ...
 *    
 *  - JRE(Java Runtime Environment) : 자바 실행 파일
 *    = JVM(Jav
 *    = JVM + Java API(lib
 *    
 ...
 *   확장자가 .class인 바이트코드(기계어)로 변역한다.
 * - 변역된 바이트코드(기계어)를 JVM의 자바 인터프리터(java.exe)를 이용해 실행한다.
 * */