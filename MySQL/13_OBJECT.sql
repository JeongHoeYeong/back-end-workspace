/*
	인덱스(INDEX)
    - SQL 명령문의 처리 속도를 향상시키기 위해 행들의 위취 정보를 가지고 있다.
...
*/
-- 테이블에서 인덱스 조회
SHOW INDEX FROM emloyee;

SELECT *
FROM INFORMATION_SCHEMA.STATISTICS
WHERE TABLE_SCHEAMA = 'kh';
...
/* 
    ...
    ex)
    - 회원탈퇴시 기존 회원테이블에 데이터 DELETE 후 곧바로 탈토된 회원들만 따로 보관하는
	  테이블에 자동으로 INSERT 처리
...
CREATE TABLE tb_product(
    ...
    brand VARCHAR(30) NOT NULL, -- 브랜드명
    price INT, -- 가격
...
VALUES('갤럭시Z 플립4', '삼성', 1000000, 10);

INSERT INTO tb_product(pname, brand, price, stock)
VALUES('아이폰15', '애플', 1200000, 20);

SELECT * FROM tb_product;
-- 2. 상품 입/출고 상세 이력 테이블 생성 (tb_pordetail)
/*
	컬럼 : dcode / 기본키
		  pcode / 외래키
          pdate / DATE / 기본값 현재날짜
          amount / INT / NOT NULL
          status / CHAR(2) / 입고, 출고 둥 중 하나
*/
...
/*
    ...
    1. 개념적 모델링
		- 엔티티(Entity) 추출
        - 엔티티 간의 관계 설정
    2. 논리적 모델링 : RED(Entity Relationship Diagram) 툴
		- 정규화 작업 (1 ~ 5) .. 3까지만 정규화!
			--> 너무 쪼개면 JOIN만 많아져요
    3. 물리적 모델링
		- 테이블 실질적으로 구성
