/*
    ...
    * MySQL에서 객체(구조) : 테이블(Tabel), 뷰(View), 인덱스(Index),
...
/*
	제약조건(CONSTRAINTS)
    - 사용자가 원하는 조건의 데이터만 유지하기 위해서 각 컬럼에 대해
	  저장될 값에 대한 제약조건을 설정
...
INSERT INTO mem_check VALUES(2, 'user02', 'pass02', '손민정', null, null, null, null);
...
-- > 부모 테이블에서(mem_grade)에서 데이터값을 삭제할 경우 어떤 문제가 발생하는지!
-- 데이터 삭제 : DELETE FROM 테이블명 WHERE 조건;

-- mem_grade 테이블에서 grade_code가 10인 등급 삭제
DELETE FROM mem_grade 
WHERE grade_code = 10;
-- > 자식테이블(member)에 10이라는 값을 사용하고 있기 때문에 삭제 X
...
-- INSERT INTO member(컬럼명, 컬럼명, ...) VALUES(컬럼값, 컬럼값, ...);
