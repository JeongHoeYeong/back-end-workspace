...
FROM employee
WHERE salary > (SELECT avg(ifnull(salary, 0)) 
                FROM employee);
                
-- 2) 3047662.6087보다 더많은 급여를 받는 사원들의 사번, 직원명, 직급코드, 급여 조회
SELECT emp_id, emp_name, job_code, format(salary, 0)
FROM employee
...
if ((emp_id IN (SELECT distinct manager_id
              	FROM employee
			    WHERE manager_id is not null)), '사수', '사원') as "구분"
FROM employee;
...
*/
-- 대리 직급임에도 과장 직급들의 최소 금여보다 많이 받는 직원의 사번, 이름, 직급, 급여 조회
-- 과장 직급들의 급여 조회
SELECT salary
...
SELECT emp_id, emp_name, coalesce(dept_code, '부서없음'), salary
FROM employee
WHERE (coalesce(dept_code, '부서없음'), salary) 
								IN (SELECT coalesce(dept_code, '부서없음'), max(salary)
								FROM employee
                                GROUP BY dept_code);
/*
	RANK 함수
...
*/
-- 사원별 급여가 높은 순서대로 순위 매겨서 순위, 사원명, 급여조회
SELECT 
