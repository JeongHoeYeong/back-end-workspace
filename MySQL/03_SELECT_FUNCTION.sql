...
-- 실습문제 -----------------------------------------------------------------
-- 1. 이메일의 kh.or.kr을 gmail.com으로 변경해서 변경 전 이름, 이메일, 변경 후 이메일 조회
...
SELECT emp_name "직원명", year(hire_date) "입사년도",
       month(hire_date) "입사월", day(hire_date) "입사일"
FROM employee
...
(salary + salary * coalesce(bonus, 0)) * 12 as "연봉"
FROM employee;
