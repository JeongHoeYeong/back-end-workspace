/*
   - 데이터(data) : 필요에 의해 수집햇지만 아직 특정 목적을 위해 정제하지 않은 값
...
-- 실습문제 ----------------------------------------------------------
-- 1. job 테이블의 모든 정보 조회alter
SELECT * FROM job;

-- 2. job 테이블의 직급 이름(job_name)이름 조회
...
-- 전화번호(phone), 입사일(hire_date)job 정보만 조회
...
/*
	...
	- 별칭을 부여할 때 띄어쓰기 혹은 혹은 특수문자가 포함될 경우
	  반드시 큰 따옴표(")로 감싸줘야 한다.
*/
...
-- 입사일이(hire-date) '1990-01-01' ~ '2001-01-01'
SELECT *
FROM employee
...
/*
   ...
   비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값이 문자가 "포함"되는걸 조회
							   (=키워드 검색)
   
   '_' : 1글자
   비교대상컬럼 LIKE '_문자' => 비교대상컬럼값에 문자앞에 무조건 1글자가 올 경우 조회
   비교대상컬럼 LIKE '__문자' => 비교대상컬럼값에 문자앞에 무조건 2글자가 올 경우 조회
...
/*
   IN
   - 비교대상컬럼값이 내가 제시한 목록 중에 칠히나는 값이 있는지
   비교대상컬럼 IN ('값1', '값2', ....)
*/
...
hire_date >= '2001-01-01' and bonus is null;

-- 6. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는
--    사번, 사원명, 급여, 보너스 포함 연봉 조회 (별칭)
