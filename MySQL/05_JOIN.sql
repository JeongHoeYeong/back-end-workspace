/*
    ...
    - 관계형 데이터베이스는 최소한의 데이터로 각각의 테이블에 담고 있음
      ...
      만약 어떤 사원이 어던 부서에 속해있는지 부서명과 같이 조회하고 싶다면?
...
/*
    ...
    - JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매칭 시킬 컬럼에 대한 조건을 기술 (ON, USING 구문)
...
-- 두 컬럼명이 같을 떄만 USING 구문 사용 가능!
SELECT emp_id, emp_name, job_code, job_name
FROM employee
JOIN job USING (job_code);

-- 참고사항! 자연조인(NATURAL JOIN) : 각 테이블마다 동일한 컬럼이 한 개만 존재할 경우
-- 실제로 추천 하지 않음!! 가급적으로 절대 쓰지 마세요
SELECT emp_id, emp_name, job_code, job_name
FROM employee
NATURAL JOIN job;
...
FROM employee, department
WHERE dpet_code = dept_id
AND dept_title = '입사관리부';
...
-- 3) FULL [OUTER] FOIN : 두 테이블이 가진 모든 행을 조회할 수 있음
SELECT emp_name, dept_title, salary, salary * 12
FROM employee
...
-- 사번, 사원명, 부서명, 직급명, 지역명, 국가명, 급여등급 조회 (employee, department, location, job,national, sal_grade)
SELECT * FROM employee;   -- emp_id, emp_name dept_code            job_code
SELECT * FROM department; -- dept_title       dept_id    location_id
SELECT * FROM job;        -- job_ name                             job_code
SELECT * FROM location;   -- local_name                  local_code           national_code
SELECT * FROM national;    -- national_name                                    national_code
SELECT * FROM sal_grade;   -- sal_level sal_max sal_min
...
-- 3. 보너스를 받은 직원들의 직원명, 보너스, 연몽, 부서명, 근무지역 조회
