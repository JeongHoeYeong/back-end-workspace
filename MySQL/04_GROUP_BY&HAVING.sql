...
-- 직급 코드(job_code)별 사원수 조회
SELECT job_code, count(*)
FROM employee
GROUP BY job_code;
-- 성별 별(남자/여자) 사원 수 조회
SELECT if(substr(emp_no, 8, 1) = 1, '남자', '여자') as '성별', count(*) as '사원 수'
FROM employee
GROUP BY 성별;
...
/*
    ...
    UNION ALL (합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행 제거X)
